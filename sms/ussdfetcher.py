#!/usr/bin/python
# -*- coding: UTF-8 -*-

import subprocess


class UssdFetcher(object):
    def __init__(self, gammu_config_file, gammu_config_section):
        self.gammu_config_file = gammu_config_file
        self.gammu_config_section = gammu_config_section

    def fetch_ussd_response(self, ussd):
        config_file = self.gammu_config_file
        config_section = self.gammu_config_section
        reply = subprocess.check_output(['gammu', '-c', config_file, '-s', config_section, 'getussd', ussd])
        return self.__parse_ussd_reply(reply)       

 
    def __parse_ussd_reply(self, ussd_reply):
        #print "ussd reply:"
        #print ussd_reply
        if "Service reply" in ussd_reply:
            lines = ussd_reply.splitlines()
            for line in lines:
                if line.startswith('Service reply'):
                    hex_response = line[line.find('0'):-1]
                    return hex_response.decode("hex")



if __name__ == "__main__":

    ussd = '*111#'
    print "Trying to call USSD code {0} ...".format(ussd)

    config_file = '/home/pi/.gammurc'
    config_section = '2'

    ussd_fetcher = UssdFetcher(config_file, config_section)
    response = ussd_fetcher.fetch_ussd_response(ussd)
    print "completed gammu ussd call, got:"
    print response
    
    #response = ""

    print "Done, bye." 
